...
- `POST /align` — JSON `{"text":"the passage","actualIpa":"...","mode":"prefix"|"window","fromWord":0,"maxWord":null}` → `{"wordIndex":4}`; `-1` when nothing trustworthy matched. `prefix` (default): actualIpa is the cumulative transcript, fitting alignment. `window`: actualIpa is a short trailing window (~2.5 s), aligned locally against the band of words `[fromWord, fromWord+25)`; returns the end of the best-matching region, `-1` if the match is too poor (silence/noise). `maxWord` (window mode) hard-caps the answer — regions ending past it aren't candidates; readers realistically skip ≤2 words between updates, so farther matches are duplicate-word coincidences. Cheap (expected phonemes are cached per text); called per chunk and per peek.
