...
- `{"type":"progress","wordIndex":6}` — reader-position update from a "peek": a trailing ~3 s window transcribed every ~0.5 s of new audio. Peeks are not part of the scored transcript; the frame carries no `ipa`. Monotonicity is the client's call (treat as a lower-bound hint).
